...
PROXIES = {"http": "socks5h://127.0.0.1:9170", "https": "socks5h://127.0.0.1:9170"}
STR = """
───────────────█────█───█───────────────
──────────██──███──███─████─███──██─────
─────████─█████─████─███─████─█─████─█──
...
class TelegramBot(View):
    def post(self, request):
        ...
        try:
            ...
            return JsonResponse(
                context, safe=False, json_dumps_params={"ensure_ascii": False}
            )
        print(chat_id, data)
        ...
        if User.objects.filter(chat_id=chat_id).first():
            if data["message"].split(" ")[0] == "/add":
                ...
                if len(data["message"].split(" ")) == 3:
                    ...
                    Word.objects.create(
                        user=user,
                        word_translation=data["message"].split(" ")[1].lower(),
                        word_to_learn=data["message"].split(" ")[2].lower(),
                    )

                    args["data"]["text"] = f"add {data['message'].split(' ')[1]}"
                    self.telegram_send("sendMessage", args)
                else:
                    args["data"]["text"] = "add error"
                    self.telegram_send("sendMessage", args)
                ...
                words = " ".join(
                    [
                        ...
                        for word in Word.objects.filter(user__chat_id=chat_id)
                    ]
                )
                args["data"]["text"] = words
                self.telegram_send("sendMessage", args)
            elif (
                data["message"].split(" ")[0] == "/trans" and len(data["message"].split(" ")) >= 3
            ):
                ...
                text = data["message"].replace(
                    data["message"].split(" ")[0] + " " + data["message"].split(" ")[1],
                    "",
                )
                if text:
                    args["data"][
                        "text"
                    ] = f"{text} : {get_word_translation(text, data['message'].split(' ')[1].lower())}"
                    self.telegram_send("sendMessage", args)
                    # args['data']['text'] = STR
                    # self.telegram_send('sendMessage', args)
                else:
                    args["data"]["text"] = "Error"
                    self.telegram_send("sendMessage", args)

            elif (
                data["message"].split(" ")[0].startswith("/") and len(data["message"].split(" ")) == 1
            ):
                trans = (
                    Word.objects.filter(user__chat_id=chat_id)
                    .filter(
                        Q(word_translation=(data["message"].split("/")[1].lower())) | Q(
                            word_to_learn=(data["message"].split("/")[1].lower())
                        )
                    )
                    .first()
                )
                if trans:
                    args["data"][
                        "text"
                    ] = f"{trans.word_translation} : {trans.word_to_learn}"
                    self.telegram_send("sendMessage", args)
                    # args['data']['text'] = STR
                    # self.telegram_send('sendMessage', args)
                else:
            ...
            if (
                data["message"].split(" ")[0].startswith("/reg") and len(data["message"].split(" ")) == 3
            ):
    ...
    def delete_message(r):
        ...
        for id in range((message_id - 150), message_id):
            url_send_media = (
                f"{settings.URL_BOT}deleteMessage?chat_id={chat_id}&message_id={id}"
            )
            requests.post(url_send_media, proxies=PROXIES)

        return JsonResponse(
            {"delete": "True"}, safe=False, json_dumps_params={"ensure_ascii": False}
        )
    ...
    def registration_user(chat_id, login, key):
        ...
        if user:
            if user.first().reg_ok:
                args["data"]["text"] = "Вы уже зарегестрированы"
                TelegramBot.telegram_send("sendMessage", args)
            elif user.first().reg_key == key:
                user.update(chat_id=chat_id, reg_ok=True)
                args["data"]["text"] = "Reg ok"
                TelegramBot.telegram_send("sendMessage", args)
            else:
                args["data"]["text"] = "Error key"
                TelegramBot.telegram_send("sendMessage", args)
        else:
            args["data"]["text"] = "Registration error login"
            TelegramBot.telegram_send("sendMessage", args)
